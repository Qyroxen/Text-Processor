...
> Text-Processor - Open source tool by AetherCodeHQ

`cli` `golang` `open-source` `processing` `text` `utilities`

## What is it?

**Text Processor** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Text-Processor.git
cd Text-Processor
...
```bash
# Basic usage
./Text-Processor --path ./target

# With options
./Text-Processor --path ./target --format json --output report.json

# Verbose mode
./Text-Processor --path ./target --verbose
```
...
### Basic scan
```bash
./Text-Processor --path ./my-project
```

### JSON report
```bash
./Text-Processor --path ./my-project --format json --output report.json
```
...
- name: Run Text Processor
  run: ./Text-Processor --path . --format json --output report.json
```
